...
INSERT INTO DISPONIBILIDAD (cedula_conductor, dia, franja_horaria, tipo_transporte, estado)
SELECT u.cedula, TRUNC(SYSDATE), '00:00-23:59', v.nivel_transporte, 'DISPONIBLE'
FROM USUARIO u
JOIN VEHICULO v ON v.cedula_dueño = u.cedula
WHERE u.rol = 'CONDUCTOR'
  AND NOT EXISTS (
        SELECT 1 FROM DISPONIBILIDAD d
        WHERE d.cedula_conductor = u.cedula
  );
COMMIT;

UPDATE DISPONIBILIDAD d
SET estado = 'ENSERVICIO'
WHERE EXISTS (
    SELECT 1
    FROM SERVICIO s
    WHERE s.cedula_conductor = d.cedula_conductor
